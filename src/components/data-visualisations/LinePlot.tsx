...
import { getLinePlot } from "../../api/dataVis";
...
const LinePlot = ({ columns, datasetId }: LinePlotProps) => {
  ...
  const { mutate, isLoading } = useMutation({
    ...
    onSuccess: (data) => {
      console.log("lineplot success", data);
    ...
    onError: (error) => {
      console.log("lineplot error", error);

      toast({
        title: "An error occurred while generating the line plot",
        status: "error",
        duration: 2500,
      });
    },
  ...
  return (
    <>
        ...
        <h2>
          <AccordionButton px={0}>
            <Box as="span" flex="1" textAlign="left">
              <Heading size={"md"}>Generate Line plot</Heading>
            </Box>
            <AccordionIcon />
          </AccordionButton>
        </h2>
      ...
      <VisualizationModal
        imgSrc={imgSrc}
        header={`Line plot of column ${formData.selectedColumn}`}
        title={formData.title}
        isOpen={isOpen}
        onClose={onClose}
      />
