...
import { getBoxplot } from "../../api/dataVis";
...
const Boxplot = ({ columns, datasetId }: BoxplotProps) => {
  ...
  const { mutate, isLoading } = useMutation({
    ...
    onSuccess: (data) => {
      console.log("boxplot success", data);
    ...
    onError: (error) => {
      console.log("boxplot error", error);

      toast({
        title: "An error occurred while generating the boxplot",
        status: "error",
        duration: 2500,
      });
    },
  ...
  return (
    <>
        ...
        <h2>
          <AccordionButton px={0}>
            <Box as="span" flex="1" textAlign="left">
              <Heading size={"md"}>Generate Boxplot</Heading>
            </Box>
            <AccordionIcon />
          </AccordionButton>
        </h2>
      ...
      <VisualizationModal
        imgSrc={imgSrc}
        header={`Boxplot of column ${formData.selectedColumn}`}
        title={formData.title}
        isOpen={isOpen}
        onClose={onClose}
      />
