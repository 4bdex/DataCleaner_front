...
import { getHistogram } from "../../api/dataVis";
...
const Histogram = ({ columns, datasetId }: HistogramProps) => {
  ...
  const { mutate, isLoading } = useMutation({
    ...
    onSuccess: (data) => {
      console.log("histogram success", data);
    ...
    onError: (error) => {
      ...
      toast({
        title: "An error occurred while generating the histogram",
        status: "error",
        duration: 2500,
      });
    },
  ...
  return (
    <>
        ...
        <h2>
          <AccordionButton px={0}>
            <Box as="span" flex="1" textAlign="left">
              <Heading size={"md"}>Generate Histogram</Heading>
            </Box>
            <AccordionIcon />
          </AccordionButton>
        </h2>
      ...
      <VisualizationModal
        imgSrc={imgSrc}
        header={`Histogram of column ${formData.selectedColumn}`}
        title={formData.title}
        isOpen={isOpen}
        onClose={onClose}
      />
